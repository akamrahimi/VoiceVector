...
class VoiceFormerAV(nn.Module):
    ...
    def __init__(self,
        ...
        self.adaptive_layer2 = nn.ModuleList()
        ...
        adaptive = [ 
            nn.Conv1d(self.video_chin,chin,kernel_size=1,stride=1),
            nn.ReLU(),
            nn.Conv1d(chin,chin,kernel_size=4,stride=1,padding=1),
            nn.ReLU()
        ]
        self.adaptive_layer.append(nn.Sequential(*adaptive))

        adaptive2 = [ 
            nn.Conv1d(chin*2,chin,kernel_size=1,stride=1),
            nn.ReLU()
        ]
        self.adaptive_layer2.append(nn.Sequential(*adaptive2))
    ...
    def forward(self, mix, feat, speaker_embedding=None):
        ...
        feat = F.interpolate(feat, size = [x.shape[2]],mode='linear', align_corners=False, recompute_scale_factor=False)
      
        x = th.cat([x,feat],1)
        
        for adapt in self.adaptive_layer2:
            x = adapt(x)
