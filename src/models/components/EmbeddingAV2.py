...
class EmbeddingAV2(nn.Module):
    ...
    def __init__(self,
        ...
        self.ECAPA = ECAPA_TDNN(chin//2, lin_neurons=192)
    ...
    def step(self, audios, feat):
        # print('audios init', audios.sum())
        with th.no_grad():

            if self.normalize:
                mono = audios.mean(dim=1, keepdim=True)
                std = mono.std(dim=-1, keepdim=True)
                audios = audios / (self.floor + std)
            else:
                std = 1
        
        # print('x init', audios.shape)       
        x = self.upsample(audios)
        # print('audios', audios.sum())
        # print('x upsample', x.shape)   
        skips = []
        for encode in self.encoder:
            x = encode(x)
            skips.append(x)
        # print('x encoder',x.sum())
        ...
        for adapt in self.adaptive_layer:
            feat = adapt(feat)
        # print('feat adapt', feat.shape)
          
        feat = feat.permute(2, 0, 1) 
        # print('feat permute', feat.shape)
        # print('feat norm', feat.sum())
        
        x = self.transformer(x, feat)
        # print('x transformer', x.sum())
        ...
        for linear in self.linear_layers:
          x = linear(x)
        # print('x linear', x.sum())
        
        # print('x linear', x.shape)
        x = x.permute(0, 2, 1)
        # print('x permute', x.shape)
        em = self.ECAPA(x)
        # print('em speaker_embedding', x.sum())
        
        # print('em output', em.shape)
        return em