...
class TransformerModel(nn.Module):
    ...
    def forward(self, aud_feat, v_feat, src_mask=None):
        ...
        encoded_src = th.cat([encoded_v_feat, encoded_aud_feat], 0)
        output = self.transformer_encoder(encoded_src, src_mask)
        output = self.decoder(output)
        return output


class AVEmbedding(nn.Module):
    """ 
    ...
    def __init__(self,
        ...
        adaptive = [ 
            nn.Conv1d(self.video_chin,chin,kernel_size=4,stride=1,padding=2),
            nn.GroupNorm(1,chin),
            nn.Dropout(0.1),
            nn.ReLU(),
        ]
        ...
        linear = [ 
            nn.Conv1d(chin,chin//2,kernel_size=4,stride=1,padding=2),
            nn.Dropout(0.1),
            nn.ReLU()
        ]
    ...
    def forward(self, mix, feat, speaker_embedding=None):
        ...
        for encode in self.encoder:
            x = encode(x)
            skips.append(x)
        print('encoded audio', x.shape)
        print('feat pre adpative layer', feat.shape)
        for adapt in self.adaptive_layer:
            feat = adapt(feat)
            
        print('feat after adpative layer', feat.shape)
